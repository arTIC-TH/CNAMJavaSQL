...
//************Modifier Client *****************
		public static void modifierClient(Connection conn, int id) {
			Statement instruction = null;
			String nom = "Bernard";
		try {
			instruction = conn.createStatement();
			instruction.executeUpdate("update clients SET nom_cli='" + nom + "'WHERE id_client=" + id + "");
			instruction.close();
		...
		nom_cli=?,prenom_cli=?,cp_cli=?,ville_cli=? WHERE id_client=?";
		prep = conn.prepareStatement(sql);
		prep.setString(1, "Dupont59");
		...
		System.out.println("SQLException: " + ex.getMessage());
		System.out.println("SQLState: " + ((SQLException) ex).getSQLState());
		System.out.println("VendorError: " + ((SQLException)
		ex).getErrorCode());
		System.out.println("Erreur : modification!");
		}